...
def coordinates_fine(
    ...
    try:        
        flwdir, resolution = read_flowdir(cfg.flwdir_fine, ftype='d8')
    ...
    try:
        points = read_shapefiles(cfg.points)
    ...
    for ID, point in tqdm(points.iterrows(), total=n_points, desc='points'):    
        try:
            ...
            for n_cell, penalty, factor, error_thr in zip(ranges, penalties, dist_factors, max_errors):
                ...
                # COMMENT
                # @casadoj: it is weird that it mixes the area error (%) with distance (number of cells).
                #           For instance, in the first iteration, it increases the % area error by 2% for each
                #           cells it deviates from the original location.
