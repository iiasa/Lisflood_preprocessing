...
maketxt = "efas_Merit_2corr.txt"

# input 5 arcmin
rootshape2 = "../shape_1min/"
rootshape3 =  "../shape_efas/"

lddname = "ldd_1min.tif"
upsname = "upArea_European_01min.nc"
# output"
# output
efas_Merit = "basins_1min.txt"
...
header += "\tlat_1min\tlon_1min\tarea_1min\tdifferentfromEFAS\n"
f = open(efas_Merit, "w")
f.write(header)
f.close()
...
rangexy = np.arange(-2./60.,2.8/60.,1./60.0)
threshold = 100
reso ="1min"
...
notsame = 0
for stationNo in range(259, len(makeshp)):

    station = makeshp[stationNo].split("\t")
    upsreal = float(station[9])
    upsmodel = float(station[21])

    lat_lisf = float(station[17])
    lon_lisf = float(station[18])


    try:
        alti = float(station[10])
    except:
        alti = -999.
    ...
    coord = [ float(station[19]), float(station[20])]
    ...
    if (upsreal >= threshold) or (upsmodel >= threshold):
        ...
        if os.path.isfile(shapefile1):
            ...
            for y in rangexy:
                for x in rangexy:
                    ...
                    if (xx>-9999) and (xx<99999999999999999):
                        ...
                        col = int((xx - 8.0) * 60 + 0.0000001)
                        row = int((51.0 - yy) * 60 + 0.0000001)
            ...
            # if difference <=10 km2 than use middle point
            if diffups <= 10.0:
            ...
            row = int((51.0 - yy) * 60 + 0.0000001)
            upsvalue = ups[row, col] / 1000000
            locx =   col / 60.0 + 8.0 + 1./120. #xx  //60.0 * 60.0+1/120
            locy = 51.0 - row / 60 - 1./120.    #yy  #//60. * 60.0+ 1/120

            col_lisf = int((lon_lisf - 8.0) * 60 + 0.0000001)
            row_lisf = int((51.0 - lat_lisf) * 60 + 0.0000001)
            locxlisf = col_lisf / 60.0 + 8.0 + 1./120.
            locylisf = 51.0 - row_lisf / 60 - 1. / 120.
            ...
            if (locxlisf != locx) or (locylisf != locy):
                ...
                gdf_bas['area_efas'] = station[15]
                gdf_bas['lat_efas']= lat_lisf
                gdf_bas['lon_efas']= lon_lisf
            ...
            gdf_bas = vectorize(subbasins.astype(np.int32), 0, flw.transform, name=stname)
            shapefile2 = rootshape2 + efas_no  + "_" + stname + "_1min.shp"

            gdf_bas['river'] = station[4]
            gdf_bas['station'] = station[4]
            gdf_bas['country'] = station[2]
            gdf_bas['no_EFAS'] = station[1]
            gdf_bas['provider_no'] = station[3]
            gdf_bas['area_provider'] = station[9]
            ...
            gdf_bas['lat_3sec']=  station[19]
            gdf_bas['lon_3sec']=  station[20]
            gdf_bas['area_3sec'] = station[21]

            gdf_bas['area_1min'] = upsvalue
            gdf_bas['lat'] = locy
            gdf_bas['lon'] = locx
            ...
            s = s + "\t" + f"{locy:8.5f}" + "\t" + f"{locx:8.5f}" + "\t" + f"{upsvalue:9.0f}"


            # add if EFAS is different
            s = s + "\t" + nots
            print (s)
            s = s + "\n"
            f = open(efas_Merit, "a")
